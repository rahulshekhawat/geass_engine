...
public:
	...
	FMatrix() { ; }
	FMatrix(const float* f);
	FMatrix(const float(*f)[4]);
	FMatrix(
	...
	friend MVector3 operator * (const MVector3& v, const FMatrix& tm);
	friend MVector4 operator * (const MVector4& v, const FMatrix& tm);
	...
	void SetRotationMatrix(const MVector3& rotation);// ���� : ����
...
inline FMatrix::FMatrix(const float* f)
{
...
inline FMatrix::FMatrix(float f11, float f12, float f13, float f14,
	float f21, float f22, float f23, float f24,
	float f31, float f32, float f33, float f34,
	float f41, float f42, float f43, float f44)
{
...
inline bool FMatrix::operator==(const FMatrix& other) const
{
	for (int i = 0; i < 16; i++)
	{
		if (FMatrix::m[i] != other.m[i]) return false;
	}
	return true;
}
...
{
	/*const FMatrix& m1 = *this, &m2 = other;
...
inline FMatrix FMatrix::operator-(const FMatrix& mat) const	// 090716, OZ
{
...
inline void FMatrix::SetTranslation(const MVector3& trans)
{
	_41 = trans.x;
	_42 = trans.y;
	_43 = trans.z;
...
inline void FMatrix::SetInverseTranslation(const MVector3& trans)
{
	_41 = -trans.x;
	_42 = -trans.y;
	_43 = -trans.z;
...
inline MVector3 FMatrix::GetTranslation() const
{
	return MVector3(_41, _42, _43);
}

inline void FMatrix::SetRotationDegrees(const MVector3& rotation)
{
	SetRotationRadians(rotation * (float)3.1415926535897932384626433832795 / 180.0);
}

inline void FMatrix::SetRotationRadians(const MVector3& rotation)
{
...
inline void FMatrix::SetScale(const MVector3& scale)
{
	_11 = scale.x;
	_22 = scale.y;
	_33 = scale.z;
...
inline void FMatrix::SetRotationMatrix(const MVector3& rotation)
{
	MakeIdentity();
	SetRotationRadians(rotation);
}

inline void FMatrix::SetTranslationMatrix(const MVector3& trans)
{
	MakeIdentity();
	SetTranslation(trans);
}

inline void FMatrix::SetScaleMatrix(const MVector3& scale)
{
	MakeIdentity();
	SetScale(scale);
}

inline void FMatrix::TransformVect(MVector3& vect) const
{
	float vector[3];
...
inline void FMatrix::TransformVectHomogeneous(MVector3& vect) const
{
	float vector[3];
...
inline void FMatrix::TransformVect(const MVector3& in, MVector3& out) const
{
...
inline void FMatrix::TransformVect(const MVector3& in, MVector4& out) const
{
	out.x = in.x * _11 + in.y * _21 + in.z * _31 + _41;
	out.y = in.x * _12 + in.y * _22 + in.z * _32 + _42;
	out.z = in.x * _13 + in.y * _23 + in.z * _33 + _43;
...
inline void FMatrix::TransformNormal(MVector3& vect) const
{
	float vector[3];
...
inline void FMatrix::TransformNormal(const MVector3& in, MVector3& out) const
{
...
inline MVector3 operator * (const MVector3& v, const FMatrix& tm)
{
	MVector3 ret;
...
inline MVector4 operator * (const MVector4& v, const FMatrix& tm)
{
	MVector4 vec4;
...
inline bool FMatrix::GetInverse(FMatrix* pOut, float* fDet = NULL) const
{
...
inline void FMatrix::SetProjectionMatrixFovRH(float fFovY, float fAspectRatio, float zNear, float zFar)
{
	...
	float xScale = yScale / fAspectRatio;	// dx9 ¹®¼­¿¡ Àß¸øµÇ¾îÀÖ´Ù
...
inline void FMatrix::SetLookAtMatrixRH(const MVector3& eye, const MVector3& at, const MVector3& up)
{
...
inline void FMatrix::SetLookAtMatrixLH(const MVector3& eye, const MVector3& at, const MVector3& up)
{
...
inline void FMatrix::TransformPlane(const MPlane& in, MPlane& out) const
{
	FMatrix matrix;
	GetInverse(&matrix);
...
inline void FMatrix::SetRotationMatrixAxis(const MVector3& axis, float radian)
{
	float c = cos(radian);
	float s = sin(radian);
...
{
	// Z, X, Y Ãà ¼ø¼­·Î °öÇÑ´Ù.
	// Ã£¾Æº» °á°ú·Î Yaw°¡ Upº¤ÅÍ ÃàÀ» ¶æÇÑ´Ù.
...
inline void FMatrix::SetLocalMatrix(const MVector3& position, const MVector3& dir, const MVector3& up, const MVector3& right)
{
	_11 = right.x;
	_12 = right.y;
	_13 = right.z;
...
inline void FMatrix::SetLocalMatrix(const MVector3& position, const MVector3& dir, const MVector3& up)
{
